...
class Btnform(FlaskForm):
    btn = SubmitField('Inicia Produção')



@app.route('/', methods=['GET','POST']) # Define a rota
def basic_interface():
    global dv, dv2

    form = Btnform()
    if (form.btn.data == True):
        teste(form.btn.data)
   
    return render_template('basic_interface.html', dv = dv, dv2 = dv2, form = form) # Sem caminho necessário (Templates)
